...
const ChoroplethMap = () => {
    ...
    const popDataUrl = 'https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world_population.csv';

    const getGeoDataCallback = useCallback(async () => {
        return d3.json(geoDataUrl);
    }, []);
    const getPopDataCallback = useCallback(async () => {
        return d3.csv(popDataUrl);
    }, []);

    const { value: geoData, status: geoDataStatus } = useAsync(getGeoDataCallback, true);
    const { value: popData, status: popDataStatus } = useAsync(getPopDataCallback, true);

    const renderFunc = useCallback(async (container) => {
        if(geoDataStatus === 'fulfilled' && popDataStatus === 'fulfilled') {
    ...
    }, [isDarkMode, geoDataStatus, geoData, popDataStatus, popData]);
    ...
    return (
        <div className='map__wrapper'>
            ...
            {(geoDataStatus === 'pending' || popDataStatus === 'pending') && <LoadingComp loadingText={'Fetching data'}/>}
            {(geoDataStatus === 'rejected' || popDataStatus === 'rejected') && <ErrorComp errorText={'Error fetching data'} />}
